          ...
          {!isLoading && templates?.length ? (
            <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {templates.map((template) => (
                <Link className="rounded-2xl border border-[#1E3022] bg-[#111E14] p-5 shadow-card transition-all hover:-translate-y-1 hover:border-[#254A2A] hover:bg-[#162419]" key={template.slug} to={ROUTES.TEMPLATE_DETAIL(template.slug)}>
    ...
    <PublicLayout>
      ...
      <section className="hero-grid pt-28 sm:pt-32">
        <div className="mx-auto max-w-4xl px-4 pb-20 sm:px-6">
          ...
          {!isLoading && template ? (
            <div className="mt-6 rounded-2xl border border-[#254A2A] bg-[#111E14] p-6 shadow-card sm:p-8">
