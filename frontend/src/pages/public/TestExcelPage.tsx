      ...
      <section className="hero-grid pt-28 sm:pt-32">
        <div className="mx-auto max-w-7xl px-4 pb-20 sm:px-6">
          ...
          <section className="mt-14" id="pilih-test">
            ...
            {!isLoading && filteredCases.length ? <div className="motion-stagger mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filteredCases.map((playCase) => <PlayCaseCard key={playCase.slug} playCase={playCase} />)}</div> : null}
          </section>
        </div>
      </section>
    </PublicLayout>
...
function HowItWorksSection() {
  ...
  return <section className="motion-section mt-16" id="cara-kerja"><div className="flex flex-col gap-2"><h2 className="font-heading text-3xl font-bold text-[#E8F0EA]">Cara Kerja Test Excel</h2><p className="text-sm leading-6 text-[#8BA98F]">Belum login tetap bisa coba. Tapi kalau mau skor dan progress tersimpan, masuk dulu ya.</p></div><div className="motion-stagger mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{steps.map((step) => <div className="motion-card rounded-2xl border border-[#1E3022] bg-[#111E14] p-5 shadow-card" key={step.title}><h3 className="font-heading text-lg font-bold text-[#E8F0EA]">{step.title}</h3><p className="mt-2 text-sm leading-6 text-[#8BA98F]">{step.body}</p></div>)}</div></section>
}
